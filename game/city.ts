...
import { Mats, signTexture, uvScaleBox, uvScale, glowTexture } from './materials';
...
/* ────────────────────────────────────────────────────────────────────────────
   ...
   The entire map derives from these four numbers, which is exactly why nothing
   ends up standing in a road: roads own the band around each grid line, the
   next 4m is sidewalk, and *every* prop and building is placed inside the
   remaining core rect. No random world-space scattering anywhere.
   ──────────────────────────────────────────────────────────────────────────── */
export const N = 6;                 // road lines per axis → 5×5 city blocks
...
export const WORLD_MIN = roadCoord(0) - ROADW / 2 - 26;
export const WORLD_MAX = roadCoord(N - 1) + ROADW / 2 + 26;
...
export interface Poi {
  ...
  x: number;
  z: number;
  kind: 'mosque' | 'market' | 'police' | 'park' | 'plaza' | 'shop' | 'home';
}

export interface Shop {
  x: number;
  z: number;
  /** Facing direction (the side you walk up to). */
  yaw: number;
  name: string;
...
export interface City {
  root: THREE.Group;
  nodes: RoadNode[];
  node(i: number, j: number): RoadNode | undefined;
...
class Builder {
   ...
   * which is what made rotated roofs lean the wrong way.
   */
  gable(mat: THREE.Material, x: number, y: number, z: number, w: number, d: number, rise: number, slopeAlongX = false): void {
    ...
    if (!slopeAlongX) {
      // ridge runs along X, slabs fall towards ±Z
      ...
      // ridge runs along Z, slabs fall towards ±X
...
export function buildCity(scene: THREE.Scene, phys: Physics, mats: Mats, preset: QualityPreset, seed = 20260805): City {
  ...
  const minimap: MinimapData = { roads: [], blocks: [], buildings: [], parks: [], water: [], labels: [] };
  const pedLoops: { x: number; z: number }[][] = [];
  const parkSpots: { x: number; z: number; yaw: number }[] = [];
  const roadSpawns: { x: number; z: number; yaw: number }[] = [];
  const shops: Shop[] = [];
  const pois: Poi[] = [];
  const itemSpots: { x: number; y: number; z: number }[] = [];
  const pickupSpots: { x: number; z: number }[] = [];
  const lampPts: number[] = [];
  const signMeshes: THREE.Mesh[] = [];

  /* ground -------------------------------------------------------------- */
  const groundGeo = new THREE.PlaneGeometry(1600, 1600);
  uvScale(groundGeo, 1600 / 6, 1600 / 6);
    ...
    for (let j = 0; j < N; j++) {
      ...
      if (preset.detail && (i + j) % 2 === 0) {
        for (const [sx2, sz2] of [[1, 1], [-1, -1]] as [number, number][]) {
          ...
          if (Math.abs(px) > WORLD_MAX - 4 || Math.abs(pz) > WORLD_MAX - 4) continue;
  ...
  for (const n of nodes) {
    const c = [node(n.i + 1, n.j), node(n.i - 1, n.j), node(n.i, n.j + 1), node(n.i, n.j - 1)];
    for (const k of c) if (k) n.nb.push(k);
  }
  ...
  for (let bi = 0; bi < N - 1; bi++) {
    for (let bj = 0; bj < N - 1; bj++) {
      ...
      streetFurniture(B, phys, rng, cx, cz, preset, lampPts, pickupSpots);
  ...
  const W = WORLD_MAX + 6;
  phys.addBox(-W - 20, -W - 20, W + 20, -W, 0, 40, KIND.Building);
  phys.addBox(-W - 20, W, W + 20, W + 20, 0, 40, KIND.Building);
  phys.addBox(-W - 20, -W, -W, W, 0, 40, KIND.Building);
  phys.addBox(W, -W, W + 20, W, 0, 40, KIND.Building);
  // countryside hedge line so the edge reads as intentional
  for (let x = -W; x <= W; x += 8) {
    B.box(mats.foliage, x, WALK_Y + 0.9, -W - 1, 8, 1.8, 1.6, 0, 3);
    B.box(mats.foliage, x, WALK_Y + 0.9, W + 1, 8, 1.8, 1.6, 0, 3);
  }
  for (let z = -W; z <= W; z += 8) {
    B.box(mats.foliage, -W - 1, WALK_Y + 0.9, z, 1.6, 1.8, 8, 0, 3);
    B.box(mats.foliage, W + 1, WALK_Y + 0.9, z, 1.6, 1.8, 8, 0, 3);
  }
  ...
  return {
    root, nodes, node, pedLoops, parkSpots, roadSpawns, shops, pois, minimap,
    itemSpots, pickupSpots, playerStart, policeStation, hospital, lampGlow,
...
function streetFurniture(
  B: Builder, phys: Physics, rng: Rng, cx: number, cz: number,
  ...
  B.cyl(TREE_MATS!.metal, cx + mid - 0.8, WALK_Y + 0.42, cz - mid + 4, 0.32, 0.28, 0.84, 8);
}

function tree(B: Builder, phys: Physics, rng: Rng, x: number, z: number, s: number): void {
  const mats = TREE_MATS!;
  const h = 2.4 * s;
  B.cyl(mats.trunk, x, WALK_Y + h / 2, z, 0.13 * s, 0.2 * s, h, 7);
  const top = WALK_Y + h;
  B.sphere(mats.foliage, x, top + 0.75 * s, z, 1.35 * s, 9, 7, 0.92);
  B.sphere(mats.foliage, x + 0.5 * s, top + 1.5 * s, z + 0.3 * s, 0.9 * s, 8, 6, 0.9);
  B.sphere(mats.foliage, x - 0.55 * s, top + 1.35 * s, z - 0.4 * s, 0.8 * s, 8, 6, 0.9);
  if (rng() > 0.5) B.sphere(mats.foliage, x + 0.1 * s, top + 2.1 * s, z - 0.1 * s, 0.7 * s, 8, 6, 0.9);
  phys.addCentered(x, z, 0.3 * s, 0.3 * s, 0, WALK_Y + h * 0.8, KIND.Prop);
}

function lamp(B: Builder, phys: Physics, x: number, z: number, ax: number, az: number, lampPts: number[]): void {
  const mats = TREE_MATS!;
  const h = 5.4;
  B.cyl(mats.metal, x, WALK_Y + h / 2, z, 0.08, 0.13, h, 6);
  const armX = ax * 0.9, armZ = az * 0.9;
  B.box(mats.metal, x + armX / 2, WALK_Y + h, z + armZ / 2, ax ? 0.9 : 0.14, 0.14, az ? 0.9 : 0.14, 0, 2);
  B.box(mats.metal, x + armX, WALK_Y + h - 0.16, z + armZ, 0.5, 0.2, 0.34, 0, 2);
  lampPts.push(x + armX, WALK_Y + h - 0.3, z + armZ);
  phys.addCentered(x, z, 0.2, 0.2, 0, WALK_Y + 1.2, KIND.Prop);
}

function bench(B: Builder, phys: Physics, x: number, z: number, rotY: number): void {
  const m = TREE_MATS!;
  B.box(m.wood, x, WALK_Y + 0.45, z, 1.9, 0.1, 0.55, rotY, 2);
  B.box(m.wood, x - Math.sin(rotY) * 0.24, WALK_Y + 0.78, z - Math.cos(rotY) * 0.24, 1.9, 0.55, 0.09, rotY, 2);
  B.box(m.metal, x - Math.cos(rotY) * 0.82, WALK_Y + 0.22, z + Math.sin(rotY) * 0.82, 0.1, 0.44, 0.5, rotY, 2);
  B.box(m.metal, x + Math.cos(rotY) * 0.82, WALK_Y + 0.22, z - Math.sin(rotY) * 0.82, 0.1, 0.44, 0.5, rotY, 2);
  phys.addCentered(x, z, 1, 0.4, 0, WALK_Y + 0.5, KIND.Prop);
}

/** Materials are passed around via a module-local handle to keep prop helpers terse. */
let TREE_MATS: Mats | null = null;
...
function cart(B: Builder, phys: Physics, x: number, z: number, yaw: number): void {
  const m = TREE_MATS!;
...
export function bindCityMaterials(m: Mats): void {
  TREE_MATS = m;
}
